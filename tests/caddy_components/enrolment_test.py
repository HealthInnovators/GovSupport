...
sys.path.append("/workspaces/GovSupport_chatbot/GovSupport_chatbot/src")
import boto3
...
user_table = "GovSupport_test_users"
...
@patch("GovSupport_core.utils.tables.users_table")
def test_user_registration(mock_user_table):
    mock_user_table.return_value = Mock()

    # --- Test User Entry --- #
    user = "john.doe@GovSupport.test.org"
    role = "Adviser"
...
def test_list_users(mock_user_table):
    ...
    # --- Test User Entry --- #
    users = [
        "john.doe@GovSupport.test.org",
        "jane.doe@GovSupport.test.org",
        "jane.smith@GovSupport.test.org",
        "john.smith@GovSupport.test.org",
    ]
    role = "Adviser"
    supervisor_space_id = "12345"
