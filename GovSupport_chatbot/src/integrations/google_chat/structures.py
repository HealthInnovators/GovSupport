...
from GovSupport_core.models import GovSupportMessageEvent, ApprovalEvent
from GovSupport_core.services.anonymise import analyse
from GovSupport_core.services.survey import get_survey, check_if_survey_required
from GovSupport_core.services import enrolment
from GovSupport_core.utils.monitoring import logger
from GovSupport_core.utils.tables import evaluation_table
...
class GoogleChat:
    def __init__(self):
        ...
        self.GovSupport = build(
            "chat",
            "v1",
            credentials=get_google_creds(os.getenv("GovSupport_SERVICE_ACCOUNT")),
        )
        self.supervisor = build(
            "chat",
            "v1",
            credentials=get_google_creds(
                os.getenv("GovSupport_SUPERVISOR_SERVICE_ACCOUNT")),
        )

    def format_message(self, event):
        """
        Receives a message from Google Chat and formats it into a GovSupport message event
        """
        ...
        message_string = event["message"]["text"].replace("@GovSupport", "")
    ...
    def send_existing_call_reminder(
        ...
        self.GovSupport.spaces().messages().create(
            parent=f"spaces/{space_id}",
    ...
    def update_message_in_adviser_space(
        ...
        match message_type:
            case "text":
                self.GovSupport.spaces().messages().patch(
    ...
    def get_edit_query_dialog(self, event):
        ...
        message_string = message_string.replace("@GovSupport", "")
    ...
    def handle_survey_response(self, event):
        ...
        GovSupport.mark_call_complete(user=user, thread_id=threadId)
    ...
    def get_post_call_survey_card(
        ...
        copy_button = {
            "text": "Copy GovSupport message",
            "onClick": {
                "action": {
                    "function": "copy_GovSupport_response",
                    "parameters": [
    ...
    def delete_message_in_adviser_space(self, space_id, message_id):
        self.GovSupport.spaces().messages().delete(
            name=f"spaces/{space_id}/messages/{message_id}"
        ).execute()
    ...
    def handle_supervisor_rejection(self, event) -> None:
        ...
        GovSupport.store_approver_event(thread_id, rejection_event)
    ...
    def finalise_GovSupport_call(self, event) -> None:
        """
        ...
        user = event["user"]["email"]
        GovSupport.mark_call_complete(user=user, thread_id=thread_id)
    ...
    def handle_edited_query(self, event) -> GovSupportMessageEvent:
        """
        ...
        event["proceed"] = True
        GovSupport_message = self.format_message(event)
        return GovSupport_message

    def handle_proceed_query(self, event) -> GovSupportMessageEvent:
        """
    ...
    def handle_control_group_query(self, event) -> GovSupportMessageEvent:
        """
        ...
        GovSupport_message_event = json.loads(message_event)
        return GovSupportMessageEvent.model_validate(GovSupport_message_event)
    ...
    def handle_supervisor_approval(self, event):
        ...
        GovSupport.store_approver_event(thread_id, approval_event)
