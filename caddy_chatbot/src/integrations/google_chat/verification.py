...
async def verify_google_chat_request(request: Request) -> Request:
    ...
    audience = os.environ.get("GovSupport_GOOGLE_CLOUD_PROJECT")
...
async def verify_google_chat_supervision_request(request: Request) -> Request:
    """
    This function verifies the incoming google chat request from GovSupport Supervisor.
    ...
    bearer_token = request.headers["Authorization"].split(" ")[1]
    audience = os.environ["GovSupport_SUPERVISOR_GOOGLE_CLOUD_PROJECT"]
