...
class MicrosoftTeams:
    ...
    def send_adviser_card(self, event: GovSupportMessageEvent, card=None):
        """
        Takes an incoming request from Teams Chat and returns a given response card
        """
        if card is None:
            card = self.messages.GovSupport_PROCESSING
    ...
    def format_message(self, event):
        """
        Receives a message from Microsoft Teams and formats it into a GovSupport message event
        """
        message_string = event["text"].replace("@GovSupport", "")
        ...
        self.send_adviser_card(GovSupport_message)
    ...
    def handle_reaction_added(self, event):
        ...
        # TODO return GovSupport message from supervisor channel to advisor
    ...
    def handle_thumbs_down(self, event, removed=False):
        """
        Handle thumbs down reaction = no approval from supervisor, GovSupport message not sent
        """
        action = "removed" if removed else "added"
        self.send_advisor_message_from_supervisor(
            event,
    ...
    # TODO make this have the details from GovSupport and the supervisor comments
