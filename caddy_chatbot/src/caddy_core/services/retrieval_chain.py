...
class GovSupportOpenSearchVectorSearch(OpenSearchVectorSearch):
...
def build_chain(GovSupport_PROMPT):
    GovSupport_retrievers = []

    for source in ["citizensadvice", "govuk", "advisernet"]:
        vectorstore = GovSupportOpenSearchVectorSearch(
        ...
        GovSupport_retrievers.append(retriever)

    lotr = MergerRetriever(retrievers=GovSupport_retrievers)
    ...
    document_chain = create_stuff_documents_chain(
        llm, prompt=GovSupport_PROMPT, document_prompt=document_formatter
    )
